...
$query = "SELECT te.*,tu.first_name,tu.last_name,p.project_name,tu.address,tu.phone,tu.email FROM tbl_invoice te LEFT JOIN tbl_project p ON p.id = te.project_id LEFT JOIN tbl_user tu ON tu.id = p.customer_id WHERE te.status = '1' and te.id = '$id'";
...
$style = '<style type="text/css">
			...
			$content = '<body onload="window.print()"><div style="min-height: 21cm;width: 21cm; font-family:Verdana, Geneva, sans-serif; font-size:13px; ">
				...
				<table width="99%"  style="font-family:Verdana, Geneva, sans-serif;font-size:12.5px;" cellpadding="0" cellspacing="0">
					<tr>
						<td rowspan="4" style="width:30%"><img src="'.($row2["company_logo"] != ''  ? '../../uploads/logo/'.$row2["company_logo"] : '../../uploads/logo-placeholder.png').'" height="100px" width="190px"></td>	
						<td style="width:50%"><b>'.$row2["company_name"].'</b><br></td>
						<td rowspan="2" style="font-size:20px;text-align:right;width:20%"><b>Invoice</b></td>
					</tr>
					<tr>	
						<td>'.($row2["company_address"] != "" ? nl2br($row2["company_address"]) : "").'</td>
					</tr>
					<tr>	
						<td>'.($row2["company_email"] != "" ? '<b>Email : </b>'.$row2["company_email"].'</b><br>' : "").'</td>
					</tr>
					<tr>	
						<td>'.($row2["company_website"] != "" ? '<b>Website : </b>'.$row2["company_website"].'</b><br>' : "").'</td>
					</tr>
				</table>
				...
				<table width="100%" style="font-family:Verdana, Geneva, sans-serif;font-size:12.5px;padding-bottom:30px" cellpadding="0" cellspacing="0">
					<tr>	
						<td style="width:15%"><b>Invoice To</b></td>
						<td style="width:45%">'.$client_name.'</td>
						<td style="width:20%;text-align:right"><b>Invoice Number </td>
						<td style="width:20%">&nbsp;&nbsp;#'.$id.'</td>
					</tr>
					<tr>	
						<td style="width:15%;vertical-align:top"><b>Address</b></td>
						<td style="width:45%">'.($row["address"] != "" ? nl2br($row["address"]) : "").'</td>
						<td style="width:20%;text-align:right;vertical-align:top"><b>Invoice Date </td>
						<td style="width:20%;vertical-align:top">&nbsp;&nbsp;'.date($data["date_format"],strtotime($row["invoice_date"])).'</td>
					</tr>
					<tr>	
						...
						<td style="width:20%">&nbsp;&nbsp;'.date($data["date_format"],strtotime($row["invoice_due_date"])).'</td>
					</tr>
					<tr>	
				...
				<center>
				<table width="99%"  style="font-family:Verdana, Geneva, sans-serif;border-collapse: collapse;font-size:12.5px;border:1px solid #ada8a8" cellpadding="0" cellspacing="0" class="custom-border">
					<tr>
					...
					$final_total = "0.00";
                    $select_ei = "SELECT tei.*,tp.product_name as pname,ts.service_name as sname,ttr.tax_rate as taxrate_id,type.measurement_type as type_nm,m.measurement_name as measure FROM tbl_invoice_item tei LEFT JOIN tbl_product tp ON tp.id = tei.product_id LEFT JOIN tbl_measurement_type type ON type.id = tei.measurement_type LEFT JOIN tbl_measurement m ON m.id = tei.measurement LEFT JOIN tbl_service ts ON ts.id = tei.product_id LEFT JOIN tbl_taxrate ttr ON ttr.id = tei.taxrate_id WHERE tei.status = '1' AND tei.invoice_id = '$id'";
                    ...
                    while($row_ei = mysqli_fetch_assoc($query_ei)){
						...
						$table_row = '<tr><td style="text-align:center" colspan="9">No Records Found</td></tr>';
					}
					$content .= $table_row;
				$tbl_end = '<tr><td colspan="8" style="text-align: right; border:1px solid #ada8a8;padding:5px"><b>Total :</b></td><td colspan=""><b>'.number_format($final_total,2).'</b></td></tr></table></center>
			</div></div></body>';
			$content .= $tbl_end;
			$html = $style.$content;
			echo $html;
