...
<?php
  ...
  $sel2 = "SELECT * FROM tbl_all_setting WHERE user_id = '$user_id'";
  $que2 = mysqli_query($mysqli,$sel2) or fatal_error( 'MySQL Error: ' . mysqli_errno($mysqli));
  $row2 = mysqli_fetch_assoc($que2);
  $date_format = $row2["date_format"];
  
?>
<header class="main-header"> 
    <a href="javascript:void(0)" class="logo"> 
    <span class="logo-mini"><b><?php echo substr($row_user['company_name'], 0, 3); ?></b></span> 
    <span class="logo-lg"><b><?php echo $row_user['company_name']; ?></b></span> </a> 
        ...
        <ul class="nav navbar-nav">
          <li class="dropdown user user-menu"> <a href="#" class="dropdown-toggle" data-toggle="dropdown"> <i class="fa fa-user"></i> <span class="hidden-xs"><?php echo $user_name; ?></span> </a>
            <ul class="dropdown-menu">
              <li class="user-header"> <img src="<?php echo (($row_user["user_image"] != "") ? "../uploads/user/".$row_user['user_image'] : "../uploads/noimage.png" )?>" class="img-circle" alt="User Image">
                <p> <?php echo $user_name; ?> <small>Member Since <?php echo date($date_format." H:i:s",strtotime($row_user["add_date"])); ?></small> </p>
              </li>
