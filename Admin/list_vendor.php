...
if (!empty($_GET['status'])) {
      switch ($_GET['status']) {
          ...
          case 'usuccess':
              $statusMsgClass = 'alert-success';
              $statusMsg      = 'Vendor Updated Successfully';
              break;
          case 'dsuccess':
              $statusMsgClass = 'alert-success';
              $statusMsg      = 'Vendor Deleted Successfully';
              break;
          case 'upstatus':
              $statusMsgClass = 'alert-success';
              $statusMsg      = 'Vendor Status Changed Successfully';
              break;
          case 'error':
              $statusMsgClass = 'alert-danger';
              $statusMsg      = 'Error In Vendor... Please Try Again';
              break;
          default:
              $statusMsgClass = '';
              $statusMsg      = '';
                        ...
                        <div class="col-md-12">
                          <div class="box box-primary">
                            <div class="box-header with-border">
                              ...
                              <a href="vendor.php"><button class="btn btn-primary pull-right btn-sm"><i class="fa fa-plus"></i>&nbsp; Add Vendor</button></a>
                            </div>
                            <div class="container">
                              ...
                              <div class="box-body">
                                <table id="datatable" class="table table-bordered table-striped" style="width:100%">
                                  ...
                                  <tbody>
                                    <?php
                                      $query = "SELECT * FROM tbl_vendor WHERE add_uid = '$uid_user'";
                                      ...
                                      if($num_rows > 0){
                                        ...
                                        while($row = mysqli_fetch_assoc($res)){
                                          ?>
                                            <tr>
                                              <td><?php echo $i; ?></td>
                                              <td><?php echo $row["first_name"]." ".$row["last_name"]; if($row["user_image"] != ""){echo '<br><a data-fancybox="gallery" href="../uploads/vendor/'.$row["user_image"].'"><img src="../uploads/vendor/'.$row["user_image"].'" height="65px" width="45%"/></a>';}?></td>
                                              <td><?php echo $row["phone"]; ?></td>
                                              <td><?php echo $row["email"]; ?></td>
                                              <td><?php echo $row["address"]; ?></td>
                                              <td>
                                              <?php 
                                                if($row["user_status"] == "1"){ echo '<span class="label label-success">Active</span>';} 
                                                if($row["user_status"] == "0"){ echo '<span class="label label-warning">Inactive</span>';}                                                 
                                              ?>
                                              </td>
                                              <td>
                                                <div class="btn-group">
                                                  <button type="button" class="btn btn-info btn-sm"><b>Choose</b></button>
                                                    ...
                                                    <ul class="dropdown-menu" role="menu">
                                                      <li><a href="view_vendor.php?VID=<?php echo encrypt($row['id']); ?>"><i class="fa fa-search" ></i> &nbsp;Details</a></li>
                                                      ...
                                                      <li><a href="vendor.php?VID=<?php echo encrypt($row['id']); ?>"><i class="fa fa-edit" ></i> &nbsp;Edit</a></li>
                  ...
                  if (confirmDelete()) {
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            if(data.result == "true"){
                                window.location.href="list_vendor.php?status="+(data.status);
                            }
                            else{
                                window.location.href="list_vendor.php?status="+(data.status);
                            }
