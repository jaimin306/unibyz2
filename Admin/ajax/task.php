    ...
    if ($cmd == "GetCategory") {
        ...
        if($sid != ""){
            ...
            if($sid == "Product")
            {
                ...
                $query = "SELECT tc.id,tc.category_name FROM tbl_category tc WHERE tc.add_uid = '$add_uid' AND tc.status = '1'";
            }
            else{
                $query = "SELECT tc.id,tc.category_name FROM tbl_service_category tc WHERE tc.status = '1' AND tc.add_uid = '$add_uid'"; 
            }    
            // echo $query;
            $res = mysqli_query($mysqli, $query) or fatal_error('MySQL Error: ' . mysqli_errno($mysqli));
    ...
    if ($cmd == "GetCategoryType") {
        ...
        if($cid != ""){
            ...
            if($type == "Product")
            {
                ...
                $query = "SELECT tpt.id,tpt.product_type_name as type_name FROM tbl_product_type tpt WHERE tpt.status = '1' AND tpt.category_id = '$cid' AND tpt.add_uid = '$add_uid'";
            }
            else{
                $query = "SELECT tst.id,tst.service_type_name as type_name FROM tbl_service_type tst WHERE tst.status = '1' AND tst.category_id = '$cid' AND  tst.add_uid = '$add_uid'";
            }    
            $res = mysqli_query($mysqli, $query) or fatal_error('MySQL Error: ' . mysqli_errno($mysqli));
            while ($row = mysqli_fetch_assoc($res)){
    ...
    if ($cmd == "GetProductService") {
        ...
        if($tid != ""){
            ...
            if($type == "Product"){
                ...
                $query = "SELECT tp.id,tp.product_name as pname FROM tbl_product tp WHERE tp.product_type_id = '$tid' AND tp.status = '1' AND tp.add_uid = '$add_uid'";
            }
            else{
                ...
                $query = "SELECT ts.id,ts.service_name as pname FROM tbl_service ts WHERE ts.service_type_id = '$tid' AND ts.status = '1' AND ts.add_uid = '$add_uid'";    
            }
            $res = mysqli_query($mysqli, $query) or fatal_error('MySQL Error: ' . mysqli_errno($mysqli));
            while ($row = mysqli_fetch_assoc($res)){
