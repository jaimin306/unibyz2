<?php
	...
	if($cmd == "GetEstimateTable"){
		...
		<table class="table table-stripped table-hover" id="tbl_est">
            ...
            $sel = "SELECT * FROM tbl_estimate_item WHERE estimate_id = '$estimate_id'";
            $res_sel = mysqli_query($mysqli,$sel) or fatal_error( 'MySQL Error: ' . mysqli_errno($mysqli));
            $num_row = mysqli_num_rows($res_sel);
            if($num_row > 0){
                    ...
                    <tr id="main_<?php echo $i; ?>">
                        <td style="width:100px">
                            <select name="sel[]" id="type_<?php echo $i; ?>" onchange="SetProduct(this.value,<?php echo $i; ?>)" class="form-control" required>
                                <option value="">Please Select</option>
                                <option value="Product" <?php if($row_sel['product_detail']=="Product"){ echo "selected"; } ?>>Product</option>
                                <option value="Service" <?php if($row_sel['product_detail']=="Service"){ echo "selected"; } ?>>Service</option>
                            </select>
                        </td>
                        <?php
                            $product_detail=$row_sel['product_detail'];
                            ...
                            if($product_detail == "Product"){                          
                                $query_service = "SELECT * FROM tbl_product WHERE status = '1' AND add_uid = '$uid_estimate'";      
                            ...
                            if($product_detail == "Service"){                           
                                $query_service = "SELECT id,service_name,hourly_rate,daily_rate FROM tbl_service WHERE status = '1' AND add_uid = '$uid_estimate'";
                                $res_service = mysqli_query($mysqli,$query_service) or fatal_error( 'MySQL Error: ' . mysqli_errno($mysqli));                         
                                while($row_service = mysqli_fetch_assoc($res_service)){ 
                                    $selected="";                                      
                                    if($row_service['id']==$row_sel['product_id']){
                        ...
                        <td> 
                            <select name="measurement_type[]" id="measurement_type_<?php echo $i; ?>" onchange="GetMeasurement(this.value,<?php echo $i; ?>)" class="form-control">
                                <option value="">Select</option>
                                <?php  
                                    $query_measu = "SELECT * FROM tbl_measurement_type WHERE status = '1' AND add_uid = '$uid_estimate' AND type='$product_detail'";        
                        ...
                        <td>
                            <input type="number" class="form-control" name="qty[]" id="qty_<?php echo $i; ?>" placeholder="Enter Item Quantity" value="<?php echo $row_sel['qty']; ?>" required="required" autocomplete="off" onkeyup="ChangePrice(<?php echo $i; ?>)"><span class="qty_check_<?php echo $i; ?>"></span> 
                        ...
                        <td style="width:100px">
                            <input type="text" class="form-control" id="total_tax_<?php echo $i; ?>" name="total_tax[]" placeholder="Enter Tax"  readonly>
                        </td>
                        <td style="width:150px">
                            <input type="text" class="form-control"  id="total_t_<?php echo $i; ?>" name="total[]" value="<?php echo ($unit_price*$row_sel['qty']); ?>" placeholder="Enter Total">
                        </td>
                        <td style="width:50px">
                            <?php if($i==0){ ?>
                            <button class="btn btn-success" type="button" onclick="AddBtn()">+</button>
